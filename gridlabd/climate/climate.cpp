						  ...
						  OBJECT *parent) ///< a pointer to the parent OBJECT
{
	...
	{
		gl_error("climate::create exception caught: %s", msg);
		rv = 0;
	}
	...
	{
		gl_error("climate::create exception caught: no info");
		rv = 0;
	}
	return rv;
}
						...
						OBJECT *parent) /// a poitner to the parent OBJECT
{
	...
	{
		gl_error("climate::init exception caught: %s", msg);
	}
	catch(const char *msg)
	{
		gl_error("climate::init exception caught: %s", msg);
	...
	{
		gl_error("climate::init exception caught: no info");
	}
	return 0;	//Indicate the failure
}
