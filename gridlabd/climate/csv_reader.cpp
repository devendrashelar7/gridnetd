...
int csv_reader::open(const char *file){
	...
	while(fgets(line, 1024, infile) > 0){
		...
		if(line[0] == '#'){	// comment
			...
			int line_rv = read_line(line, sample_ct);
			if(0 == line_rv){
...
int csv_reader::read_line(char *line, const long int sct){
	int done = 0;
	int col = 0;
	char buffer[1024];
	char *token = 0;
...
//	OBJECT *my = 0;
	...
	if(weather_last != 0){
		...
		t2 = sample->month * 31*24*60*60 +
			 ...
			 sample->second;
		if(t1 >= t2){
			gl_warning("csv_reader::read_line ~ sample %i does not advance in time (%lli) and has been discarded", sct, t2);
			delete sample;
			return 2;
		}
	}
