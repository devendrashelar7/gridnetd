					  ...
					  PASSCONFIG pass) /**< the pass configuration */
{
	...
	if(fabs(plc_time)<fabs(sync_time))
	{
		if (sync_time < 0)            // if sync is soft, then doesn’t matter, just take plc’s hard/soft (most common occurrence)
            sync_time = plc_time;
		else if (plc_time < 0)        // if sync is a hard timestamp and plc is soft, then set to hard(plc time) to preserve the hard nature of sync_time
            sync_time = -plc_time;
        else                          // in case both are hard or zero
            sync_time = plc_time;
	}

	/* compute valid_to time */
	if(fabs(sync_time)>TS_MAX)
					  ...
					  PASSCONFIG pass) /**< the pass configuration */
{
	...
	int n = global_iteration_limit;
	int t3 = t2;
	do {
		/* don't call sync beyond valid horizon */
		t2 = _object_sync(obj,(ts<(obj->valid_to>0?obj->valid_to:TS_NEVER)?ts:obj->valid_to),pass);

		// limit the number of retries on valid_to
		if ( t3==t2 )
			n--;
		else
		{
			n=global_iteration_limit;
			t3=t2;
		}
	} while ( n>0 && t2>0 && ts>(t2<0?-t2:t2) && t2<TS_NEVER );
	return n==0 ? TS_ZERO : t2;
}
