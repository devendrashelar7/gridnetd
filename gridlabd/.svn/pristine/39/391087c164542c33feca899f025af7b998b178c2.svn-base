...
EXPORT int init_histogram(OBJECT *obj)
{
	...
	catch (char *msg)
	{
		...
		gl_error("%s (histogram:%d): %s", (obj->name ? obj->name : "(unnamed)"), obj->id, msg);
		return 0; 
	}
...
EXPORT TIMESTAMP sync_histogram(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	...
	try {
		...
		gl_error("%s (histogram:%d): %s", (obj->name ? obj->name : "(unnamed)"), obj->id, error);
		return 0; 
	} catch (...) {
		//GL_THROW("%s (histogram:%d): %s", (obj->name ? obj->name : "(unnamed)"), obj->id, "unknown exception");
		gl_error("%s (histogram:%d): %s", (obj->name ? obj->name : "(unnamed)"), obj->id, "unknown exception");
		return 0;
	}
	return TS_NEVER;
}
