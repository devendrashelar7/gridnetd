...
EXPORT int init_inverter(OBJECT *obj, OBJECT *parent) 
{
	...
	{
		gl_error("init_inverter %s (%s:%d): %s", obj->name?obj->name:"unnamed", obj->oclass->name, obj->id, msg);
	}
	catch (...)
	{
		gl_error("init_inverter %s (%s:%d): unknown exception", obj->name?obj->name:"unnamed", obj->oclass->name, obj->id);
	}
	return 0;
}
...
EXPORT TIMESTAMP sync_inverter(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	...
	{
		gl_error("inverter %s (%s:%d): %s", obj->name, obj->oclass->name, obj->id, msg);
	}
	catch (...)
	{
		gl_error("inverter %s (%s:%d): unknown exception", obj->name, obj->oclass->name, obj->id);
	}
