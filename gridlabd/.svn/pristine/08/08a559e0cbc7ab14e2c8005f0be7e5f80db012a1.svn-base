...
{
	solar *test_solar = OBJECTDATA(obj,solar);
	try {
		return test_solar->init(obj->parent);
	}
...
EXPORT TIMESTAMP sync_solar(OBJECT *obj, TIMESTAMP t1, PASSCONFIG pass)
{
	...
	{
		gl_error("solar %s (%s:%d): %s", obj->name, obj->oclass->name, obj->id, msg);
	}
	catch (...)
	{
		gl_error("solar %s (%s:%d): unknown exception", obj->name, obj->oclass->name, obj->id);
	}

	return TS_INVALID;	//Return invalid time - should only get here for errors
}
