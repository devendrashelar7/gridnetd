	...
	for ( obj=object_get_first() ; obj!=NULL ; obj=object_get_next(obj) )
	{
		if ( obj->flags&OF_DELTAMODE )
		{
			if ( !obj->oclass->update )
				output_debug("object '%s' requested deltamode updates but the class '%s' does not export the update function", object_name(obj), obj->oclass->name);
			delta_objectcount++;
			if ( obj->rank > toprank ){
				toprank = obj->rank;
			}
		...
		while (delta_iteration_remaining>0) /* Iterate on this delta timestep */
		{
			...
			for ( n=0 ; n<delta_objectcount ; n++ )
			{
				...
				if ( oclass->update && !oclass->update(obj,timestep,delta_iteration_count) ){
					output_error("delta_update(): update failed for object \'%s\'", object_name(obj));
					/* TROUBLESHOOT
					   A module failed to update correctly while operating in deltamode.
					   Generally, this is an internal error and should be reported to the GridLAB-D developers.
					 */
					return DT_INVALID;
				}
