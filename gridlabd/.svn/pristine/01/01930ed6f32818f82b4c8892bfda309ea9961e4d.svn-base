...
load_tracker::load_tracker(MODULE *mod)
{
	...
	if (oclass==NULL)
	{
		...
		oclass = gl_register_class(mod,"load_tracker",sizeof(load_tracker),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT);
		if (oclass==NULL)
...
EXPORT int create_load_tracker(OBJECT **obj, OBJECT *parent)
{
	...
	{
		gl_error("create_load_tracker: %s", msg);
	}
	return 0;
}
...
EXPORT int init_load_tracker(OBJECT *obj)
{
	...
	{
		gl_error("%s (load_tracker:%d): %s", my->get_name(), my->get_id(), msg);
		return 0; 
	}
}
...
EXPORT TIMESTAMP sync_load_tracker(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	...
	try {
		...
		gl_error("%s (load_tracker:%d): %s", pObj->get_name(), pObj->get_id(), error);
		return TS_INVALID; 
	} catch (...) {
		gl_error("%s (load_tracker:%d): %s", pObj->get_name(), pObj->get_id(), "unknown exception");
		return TS_INVALID;
	}
}