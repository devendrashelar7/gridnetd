...
EXPORT int create_plc(OBJECT **obj, OBJECT *parent)
{
	...
	*obj = gl_create_object(plc::oclass);
	if (*obj!=NULL)
	{
		plc *my;
		try{
			my = OBJECTDATA(*obj,plc);
			gl_set_parent(*obj,parent);
			my->create();
			rv = 1;
		} catch (char *msg){
			gl_error("plc::create exception caught: %s", msg);
			rv = 0;
...
EXPORT int init_plc(OBJECT *obj)
{
	...
	if (obj!=NULL)
	{
		...
		try {
			...
			gl_error("plc::init exception caught: %s", msg);
			rv = 0;
		} catch (const char *msg){
			gl_error("plc::init exception caught: %s", msg);
			rv = 0;
...
EXPORT TIMESTAMP sync_plc(OBJECT *obj, TIMESTAMP t0)
{
	TIMESTAMP t1;
	try{
		t1 = OBJECTDATA(obj,plc)->sync(obj->clock,t0);
		obj->clock = t0;
	} catch (char *msg){
		gl_error("plc::sync exception caught: %s", msg);
		t1 = TS_INVALID;
	} catch (const char *msg){
		gl_error("plc::sync exception caught: %s", msg);
		t1 = TS_INVALID;
