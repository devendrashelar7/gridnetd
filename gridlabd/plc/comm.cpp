...
EXPORT int create_comm(OBJECT **obj, OBJECT *parent)
{
	...
	*obj = gl_create_object(comm::oclass);
	if (*obj!=NULL){
		try{
			rv = OBJECTDATA(*obj,comm)->create();
		} catch (char *msg){
			gl_error("comm::create exception caught: %s", msg);
			rv = 0;
...
EXPORT int init_comm(OBJECT *obj)
{
	...
	if (obj!=NULL){
		try{
			rv = OBJECTDATA(obj,comm)->init(obj->parent);
		} catch (char *msg){
			gl_error("comm::init exception caught: %s", msg);
			rv = 0;
		} catch (const char *msg){
			gl_error("comm::init exception caught: %s", msg);
			rv = 0;
...
EXPORT TIMESTAMP sync_comm(OBJECT *obj, TIMESTAMP t0)
{
	TIMESTAMP t1;
	try{
		t1 = OBJECTDATA(obj,comm)->sync(t0);
	}  catch (char *msg){
		gl_error("comm::sync exception caught: %s", msg);
		t1 = TS_INVALID;
	} catch (const char *msg){
		gl_error("comm::sync exception caught: %s", msg);
		t1 = TS_INVALID;
