...
protected:
public:
	enum {FULL=0,REAL=1,IMAGINARY=2,MAGNITUDE=3,ANGLE=4}; //If you want to look at only a part of 
	enumeration operation;
	enum {ONCE_FALSE=0, ONCE_TRUE=1, ONCE_DONE=2};
	enumeration once;				//  the complex number.
	complex once_value;
	enum {ASSERT_TRUE=1, ASSERT_FALSE, ASSERT_NONE};
	enumeration status; //Assert whether the target value should be
