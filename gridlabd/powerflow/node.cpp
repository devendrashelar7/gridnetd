...
int node::create(void)
{
	...
	// Only used in capacitors and meters, at this time, but put into node for future functionality (maybe with reliability?)
...
int *node::NR_populate(void)
{
		...
		if (restoration_object != NULL)
		{
			...
			else	//Only child, ensure we set it as zero
			{
				...
				{
					NR_busdata[NR_node_reference].Child_Nodes = NULL;
				}
			}
		}
