...
EXPORT TIMESTAMP sync_overhead_line_conductor(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	return TS_NEVER;

	/*try {
		overhead_line_conductor *pObj = OBJECTDATA(obj,overhead_line_conductor);
		link pLink = (link) *pObj);
		TIMESTAMP t1 = TS_NEVER;
		switch (pass) {
		case PC_PRETOPDOWN:
			return pLink.presync(t0);
		case PC_BOTTOMUP:
			return pLink.sync(t0);
		case PC_POSTTOPDOWN:
			t1 = pLink.postsync(t0);
			obj->clock = t0;
			return t1;
		default:
