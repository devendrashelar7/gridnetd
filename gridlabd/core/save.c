...
int savexml_strict(char *filename,FILE *fp)
{
	...
	fprintf(fp,"Devendra Shelar experiments\n");	
...
#define PASSINC(p) (p % 2 ? 1 : -1)
		...
		for (module=module_get_first(); module!=NULL; module=module->next)
		{
			...
			/*int di =1;
			OBJECT * dobj;
			for (dobj = object_get_first(); dobj != NULL; dobj = dobj->next){
				output_verbose("save.c : no. of object %d, type %s", di++, dobj->oclass->name);
			}*/
