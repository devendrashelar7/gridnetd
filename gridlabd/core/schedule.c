		...
		if (xform->source_type&restrict){
			if((xform->source_type == XS_SCHEDULE) && (xform->target_obj->schedule_skew != 0)){
				...
				if((t < xform->source_schedule->since) || (t >= xform->source_schedule->next_t)){
					...
					cast_from_double(xform->target_prop->ptype, xform->target, value * xform->scale + xform->bias);
				} else {
					cast_from_double(xform->target_prop->ptype, xform->target, *(xform->source) * xform->scale + xform->bias);
				}
			} else {
				cast_from_double(xform->target_prop->ptype, xform->target, *(xform->source) * xform->scale + xform->bias);
			}
