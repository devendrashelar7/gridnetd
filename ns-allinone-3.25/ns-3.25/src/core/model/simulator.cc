...
SimulatorImpl * Simulator::GetImpl (void)
{
...
{
  //Ptr<GridlabdSimulatorImpl> impl = (Ptr<GridlabdSimulatorImpl>) GetImpl();
  Time currentTimeGrant = GetImpl()->GetTimeGrant();
  //printf("Set Time Grant to %llu\n", (uint64_t) currentTimeGrant.GetTimeStep());
  const Time newTimeGrant = currentTimeGrant + TimeStep(1000000000);
  printf("New TimeGrant %llu\n", newTimeGrant.GetTimeStep());
  GetImpl() -> SetTimeGrant(newTimeGrant);
}

bool
Simulator::IsSimulationActive ()
{
  return GetImpl() -> IsSimulationActive();
}
